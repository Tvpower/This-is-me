    ...
    <section id="home" className="min-h-screen flex items-center relative py-20">
      <div className="absolute inset-0 overflow-hidden opacity-20">
        {Array.from({ length: 40 }).map((_, i) => (
          <div
            key={i}
            className="absolute text-xs md:text-sm font-mono opacity-30"
            style={{
              top: `${Math.random() * 100}%`,
              left: `${Math.random() * 100}%`,
              transform: `rotate(${Math.random() * 360}deg)`,
              color: Math.random() > 0.7 ? "rgb(255,0,0)" : "rgb(255,255,255)",
            }}
          >
      ...
      {showWarning && (
        <div className="absolute top-10 left-0 right-0 flex justify-center z-20">
          <div className="bg-red-900/80 border border-red-500 px-4 py-2 animate-pulse">
            <GlitchText
      ...
      {/* Coordinates overlay */}
      {showCoordinates && (
        <div className="absolute bottom-10 left-10 z-20 font-mono text-xs text-green-500 opacity-80">
      ...
      <div className="grid md:grid-cols-2 gap-8 items-center relative z-10">
        <div className="space-y-6 max-w-xl">
          ...
          <p className={cn("text-lg md:text-xl text-gray-400 font-mono", scrambledText && "text-red-500")}>
            <GlitchText
              text="Creating digital experiences from the fragments of reality. Code is my language, chaos is my canvas."
        ...
        <div className="relative">
          <div className="absolute inset-0 border-2 border-dashed border-red-500/20 -translate-x-4 translate-y-4" />
          ...
          {/* Data readouts */}
          <div className="absolute -bottom-6 left-0 right-0 font-mono text-xs text-green-500 flex justify-between px-4">
